...
public class WaitUtils {
    ...
    /*public static boolean waitForSelectElement(WebDriver driver, List<Select> elements, int maxWait) {
        boolean statusOfElementToBeReturned = false;
        long startTime = StopWatch.startTime();
        WebDriverWait wait = new WebDriverWait(driver, maxWait);
        try {
        	Select waitElement = (Select) wait.until(ExpectedConditions.elementToBeClickable());
            if (waitElement.isDisplayed() && waitElement.isEnabled()) {
                statusOfElementToBeReturned = true;
                Log.event("List Element is displayed:: " + elements.toString());
            }
